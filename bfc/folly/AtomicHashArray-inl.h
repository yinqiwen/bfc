...
namespace bfc {
static constexpr short NO_NEW_INSERTS = 1;
static constexpr short NO_PENDING_INSERTS = 2;
...
AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::AtomicHashArray(
    size_t capacity, KeyT emptyKey, KeyT lockedKey, KeyT erasedKey, double _maxLoadFactor, uint32_t cacheSize)
    : capacity_(capacity),
      maxEntries_(size_t(_maxLoadFactor * capacity_ + 0.5)),
      kEmptyKey_(emptyKey),
      kLockedKey_(lockedKey),
      kErasedKey_(erasedKey),
      kAnchorMask_(folly::nextPowTwo(capacity_) - 1),
      numEntries_(0, cacheSize),
      numPendingEntries_(0, cacheSize),
      isFull_(0),
      numErases_(0) {
  if (capacity == 0) {
    folly::throw_exception<std::invalid_argument>("capacity");
  }
}
...
typename AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::SimpleRetT
AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::findInternal(
    const LookupKeyT key_in) {
  checkLegalKeyIfKey<LookupKeyT>(key_in);

  for (size_t idx = keyToAnchorIdx<LookupKeyT, LookupHashFcn>(key_in), numProbes = 0;;
...
bool AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::tryInsert(
    value_type* cell, key_type key_new, bool erased, ArgTs&&... vCtorArgs) {
  if (tryLockCell(cell, erased)) {
    // KeyT key_new;
    ...
    try {
      ...
      new (const_cast<mapped*>(&cell->second)) ValueT(std::forward<ArgTs>(vCtorArgs)...);
    ...
    // (we just inserted it)
    DCHECK(relaxedLoadKey(*cell) == key_new || relaxedLoadKey(*cell) == kErasedKey_);
    --numPendingEntries_;
    if (!erased) {
 ...
 */
template <class KeyT, class ValueT, class HashFcn, class EqualFcn, class Allocator, class ProbeFcn, class KeyConvertFcn>
template <typename LookupKeyT, typename LookupHashFcn, typename LookupEqualFcn, typename LookupKeyToKeyFcn,
          typename... ArgTs>
typename AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::SimpleRetT
AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::insertInternal(
    LookupKeyT key_in, ArgTs&&... vCtorArgs) {
  constexpr int kMaxEraseCells = 16;
  // const short NO_NEW_INSERTS = 1;
  // const short NO_PENDING_INSERTS = 2;
  checkLegalKeyIfKey<LookupKeyT>(key_in);
  folly::small_vector<size_t, kMaxEraseCells> erased_cells;
  size_t idx = keyToAnchorIdx<LookupKeyT, LookupHashFcn>(key_in);
  size_t numProbes = 0;
  KeyT key_new = LookupKeyToKeyFcn()(key_in);
  typedef typename std::remove_const<LookupKeyT>::type LookupKeyTNoConst;
  constexpr bool kAlreadyChecked = std::is_same<KeyT, LookupKeyTNoConst>::value;
  if (!kAlreadyChecked) {
    checkLegalKeyIfKey(key_new);
  ...
  for (;;) {
    ...
    if (relaxedLoadKey(*cell) == kEmptyKey_) {
      ...
      if (isFull_.load(std::memory_order_acquire)) {
        ...
        // a new entry.)
        folly::detail::atomic_hash_spin_wait([&] {
          return (isFull_.load(std::memory_order_acquire) != NO_PENDING_INSERTS) &&
        ...
        if (relaxedLoadKey(*cell) == kEmptyKey_) {
          ...
          if (erased_cells.empty()) {
            return SimpleRetT(capacity_, false);
          } else {
            for (auto erased_idx : erased_cells) {
              value_type* erased_cell = &cells_[erased_idx];
              if (tryInsert(erased_cell, key_new, true, std::forward<ArgTs>(vCtorArgs)...)) {
                return SimpleRetT(erased_idx, true);
              }
            }
            return SimpleRetT(capacity_, false);
          }
        ...
        if (tryInsert(cell, key_new, false, std::forward<ArgTs>(vCtorArgs)...)) {
          return SimpleRetT(idx, true);
        }
    ...
    if (kLockedKey_ == acquireLoadKey(*cell)) {
      folly::detail::atomic_hash_spin_wait([&] { return kLockedKey_ == acquireLoadKey(*cell); });
    }
    ...
    if (LookupEqualFcn()(thisKey, key_in)) {
      ...
      continue;
    } else if (thisKey == kErasedKey_) {
      erased_cells.push_back(idx);
    }
 ...
 */
template <class KeyT, class ValueT, class HashFcn, class EqualFcn, class Allocator, class ProbeFcn, class KeyConvertFcn>
size_t AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::erase(KeyT key_in) {
  CHECK_NE(key_in, kEmptyKey_);
  CHECK_NE(key_in, kLockedKey_);
  CHECK_NE(key_in, kErasedKey_);

  for (size_t idx = keyToAnchorIdx(key_in), numProbes = 0;; idx = ProbeFcn()(idx, numProbes, capacity_)) {
    DCHECK_LT(idx, capacity_);
    value_type* cell = &cells_[idx];
    ...
    if (EqualFcn()(currentKey, key_in)) {
      ...
      if (cellKeyPtr(*cell)->compare_exchange_strong(expect, kErasedKey_)) {
        int64_t n = numErases_.fetch_add(1, std::memory_order_relaxed);
        if (n > 0) {
          // if (numEntries_.readFast() - n < maxEntries_) {
          //   int64_t expect = NO_PENDING_INSERTS;
          //   isFull_.compare_exchange_strong(expect, 0, std::memory_order_acq_rel);
          // }
        }
...
AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::create(size_t maxSize,
                                                                                             const Config& c) {
  ...
  try {
    new (mem)
        AtomicHashArray(capacity, c.emptyKey, c.lockedKey, c.erasedKey, c.maxLoadFactor, c.entryCountThreadCacheSize);
  } catch (...) {
...
template <class KeyT, class ValueT, class HashFcn, class EqualFcn, class Allocator, class ProbeFcn, class KeyConvertFcn>
void AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::destroy(AtomicHashArray* p) {
  assert(p);
...
template <class KeyT, class ValueT, class HashFcn, class EqualFcn, class Allocator, class ProbeFcn, class KeyConvertFcn>
void AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::clear() {
...
struct AtomicHashArray<KeyT, ValueT, HashFcn, EqualFcn, Allocator, ProbeFcn, KeyConvertFcn>::aha_iterator
    : folly::detail::IteratorFacade<aha_iterator<ContT, IterVal>, IterVal, std::forward_iterator_tag> {
  ...
  explicit aha_iterator(ContT* array, size_t offset) : aha_(array), offset_(offset) {}
 ...
 private:
  ...
  friend class folly::detail::IteratorFacade<aha_iterator, IterVal, std::forward_iterator_tag>;
  ...
  bool equal(const aha_iterator& o) const { return aha_ == o.aha_ && offset_ == o.offset_; }

  IterVal& dereference() const { return aha_->cells_[offset_]; }

  bool isValid() const {
    KeyT key = acquireLoadKey(aha_->cells_[offset_]);
    return key != aha_->kEmptyKey_ && key != aha_->kLockedKey_ && key != aha_->kErasedKey_;
  }
