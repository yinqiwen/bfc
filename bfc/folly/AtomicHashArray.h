...
inline void checkLegalKeyIfKeyTImpl(NotKeyT /* ignored */, KeyT /* emptyKey */, KeyT /* lockedKey */,
                                    KeyT /* erasedKey */) {}

template <typename KeyT>
inline void checkLegalKeyIfKeyTImpl(KeyT key_in, KeyT emptyKey, KeyT lockedKey, KeyT erasedKey) {
...
template <class KeyT, class ValueT, class HashFcn = std::hash<KeyT>, class EqualFcn = std::equal_to<KeyT>,
          class Allocator = std::allocator<char>, class ProbeFcn = AtomicHashArrayLinearProbeFcn,
          class KeyConvertFcn = folly::Identity>
class AtomicHashMap;
...
class AtomicHashArray {
  static_assert((std::is_convertible<KeyT, int32_t>::value || std::is_convertible<KeyT, int64_t>::value ||
 ...
 public:
  ...
  iterator find(LookupKeyT k) {
    return iterator(this, findInternal<LookupKeyT, LookupHashFcn, LookupEqualFcn>(k).idx);
  }

  template <typename LookupKeyT = key_type, typename LookupHashFcn = hasher, typename LookupEqualFcn = key_equal>
  const_iterator find(LookupKeyT k) const {
    return const_cast<AtomicHashArray*>(this)->find<LookupKeyT, LookupHashFcn, LookupEqualFcn>(k);
  }
   ...
   */
  std::pair<iterator, bool> insert(const value_type& r) { return emplace(r.first, r.second); }
  std::pair<iterator, bool> insert(value_type&& r) { return emplace(r.first, std::move(r.second)); }
   ...
   */
  template <typename LookupKeyT = key_type, typename LookupHashFcn = hasher, typename LookupEqualFcn = key_equal,
            typename LookupKeyToKeyFcn = key_convert, typename... ArgTs>
  std::pair<iterator, bool> emplace(LookupKeyT key_in, ArgTs&&... vCtorArgs) {
    SimpleRetT ret = insertInternal<LookupKeyT, LookupHashFcn, LookupEqualFcn, LookupKeyToKeyFcn>(
  ...
  size_t size() const { return numEntries_.readFull() - numErases_.load(std::memory_order_relaxed); }

  bool empty() const { return size() == 0; }

  bool isFull() const { return isFull_.load(); }

  size_t numEntries() const { return numEntries_.readFull(); }
  size_t numErases() const { return numErases_.load(); }
  ...
  const_iterator findAt(uint32_t idx) const { return const_cast<AtomicHashArray*>(this)->findAt(idx); }
  ...
  uint32_t getEntryCountThreadCacheSize() const { return numEntries_.getCacheSize(); }
 ...
 private:
  ...
  bool tryInsert(value_type* cell, key_type key_new, bool erased, ArgTs&&... vCtorArgs);

  template <typename LookupKeyT = key_type, typename LookupHashFcn = hasher, typename LookupEqualFcn = key_equal,
            typename LookupKeyToKeyFcn = folly::Identity, typename... ArgTs>
  ...
  static std::atomic<KeyT>* cellKeyPtr(const value_type& r) {
    ...
    static_assert(sizeof(std::atomic<KeyT>) == sizeof(KeyT), "std::atomic is implemented in an unexpected way for AHM");
    return const_cast<std::atomic<KeyT>*>(reinterpret_cast<std::atomic<KeyT> const*>(&r.first));
  }

  static KeyT relaxedLoadKey(const value_type& r) { return cellKeyPtr(r)->load(std::memory_order_relaxed); }

  static KeyT acquireLoadKey(const value_type& r) { return cellKeyPtr(r)->load(std::memory_order_acquire); }
  ...
  // accurate.
  folly::ThreadCachedInt<uint64_t> numEntries_;         // Successful key inserts
  folly::ThreadCachedInt<uint64_t> numPendingEntries_;  // Used by insertInternal
  ...
  AtomicHashArray(size_t capacity, KeyT emptyKey, KeyT lockedKey, KeyT erasedKey, double maxLoadFactor,
                  uint32_t cacheSize);
  ...
  inline bool tryLockCell(value_type* const cell, bool erased = false) {
    KeyT expect = erased ? kErasedKey_ : kEmptyKey_;
    return cellKeyPtr(*cell)->compare_exchange_strong(expect, kLockedKey_, std::memory_order_acq_rel);
  }
...
#include "bfc/folly/AtomicHashArray-inl.h"
